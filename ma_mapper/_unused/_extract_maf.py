...
def extract_maf(maf_file, chrom, start, end, strand,target_species = "Homo_sapiens", count_arg:_COUNTARG = 'common',age=None, age_arg:_AGEARG = None, species_age_table_file = None):
    ...
    if age_arg == 'extract':
        ...
        return output_array
#%%
def maf_io(metadata: pd.DataFrame, 
           maf:str,
           ...
           age_list: List|str|pd.DataFrame|None=None):
    ...
    if output_dir is None:
        if isinstance(metadata_local, str):
            output_dir = '/'.join(str.split(metadata_local, sep ='/')[:-1])
        else: 
            output_dir = os.path.dirname(os.path.abspath(__file__))

    logger.info('extract from maf target: '+ maf)
    if custom_id == False:
        meta_id = 'sample_n'+ metadata_local.index.astype(str)
        metadata_local['meta_id'] = meta_id
    else:
        metadata_local['meta_id'] = metadata_local.iloc[:,4]
        meta_id = metadata_local.iloc[:,4].unique()
    ...
    if age_arg is None:
        ...
        if age_list is None:
            ...
            if 'te_age' in metadata_local:
                ...
                from .. import mapper
                metadata_age=mapper.match_age_to_id_metadata(metadata_local)
