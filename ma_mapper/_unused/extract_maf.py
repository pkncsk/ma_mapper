...
def extract_maf(maf_file:str, 
                ...
                divergence_ref:pd.DataFrame = None):
    #print(target_species,chrom, start, end, strand)
    if age_arg is not None:
        if isinstance(divergence_ref, str):
            if (os.path.isfile(divergence_ref) == True):
                species = pd.read_table(divergence_ref, sep='\t')
            else:
                logger.error('metadata file not found')
        else:
            species = divergence_ref
...
def maf_io(metadata: pd.DataFrame|str, 
    ...
    if age_arg is None:
        ...
        with ProcessPoolExecutor(max_workers=40) as executor:
            results  = executor.map(extract_maf, maf_call_list, chrom_list, start_list, end_list, strand_list, repeat(target_species) ,repeat(count_arg), repeat(age_list), repeat(age_arg), repeat(divergence_ref))
        ...
        with ProcessPoolExecutor(max_workers=40) as executor:
            results  = executor.map(extract_maf, maf_call_list, chrom_list, start_list, end_list, strand_list, repeat(target_species) ,repeat(count_arg), age_list_local, repeat(age_arg), repeat(divergence_ref))
    maf_out = []
