...
def overlay_heatmap_handler(heatmap_grid, overlay, cmap, idx, data_vmax,data_vmin):
    vmax = data_vmax[idx-1] if data_vmax is not None else None
    vmin = data_vmin[idx-1] if data_vmin is not None else None
    heatmap_grid.imshow(overlay, aspect='auto', cmap=cmap, vmax=vmax, vmin=vmin)
