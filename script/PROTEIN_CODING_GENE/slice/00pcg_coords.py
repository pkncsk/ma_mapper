...
# %%
set_length_limit = 500 #front 500 back 500
coords = []
for idx in gencode_protcoding.index:
    gene_by_idx=gencode_protcoding[gencode_protcoding.index==idx]
    metadata_id=gene_by_idx['#name'].values[0]
    ...
    for idx in range(len(cds_start_list)):
        ...
        total_cds_length = total_cds_length + fragment_length     


    if total_cds_length >=set_length_limit*2:
        accumulate_length = 0
        front = []
        for i in range(len(cds_start_list)):
            fragment_length = cds_end_list[i] - cds_start_list[i]
            if accumulate_length+fragment_length < 500:
                accumulate_length = accumulate_length+fragment_length
                ...
                leftover = set_length_limit - accumulate_length
...
testgene_coord = pd.DataFrame(coords, columns=['chrom','start','end','strand','id'])
testgene_bed = testgene_coord[['chrom','start','end']]
testgene_bed['name'] = testgene_coord['id']
testgene_bed['score'] = 10
testgene_bed['strand'] = testgene_coord['strand']
output_filepath = '/home/pc575/rds/rds-mi339-kzfps/users/pakkanan/phd_project_development/data/_mapper_output/hg38_main/gene_ref/protein_coding_sequences_sliced.txt'
#%%
testgene_bed.to_csv(output_filepath, index=False,sep='\t', header=False)
#%%
set_length = 500 #front 500 back 500
test_length = []
seq_records = []
gene_by_idx=gencode_protcoding[gencode_protcoding['#name']=='ENST00000616016.5']
metadata_id=gene_by_idx['#name'].values[0]
print(metadata_id)
start_list=gene_by_idx.exonStarts.str.split(',').values[0]
end_list=gene_by_idx.exonEnds.str.split(',').values[0]
