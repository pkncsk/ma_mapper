...
genomewide_data_filepath = '/home/pc575/rds/rds-kzfps-XrHDlpCeVDg/users/pakkanan/data/resource/annotation/homer_known_motif_hg38/CLOCK(bHLH).bed'
...
#%%
binding_indices_right = np.unique(np.where(motif_matrix[:, 227] != 0)[0])
nonbinding_indices_right=list(set(np.arange(motif_matrix.shape[0])) - set(binding_indices_right))
binding_indices_left = np.unique(np.where(motif_matrix[:, 65] != 0)[0])
