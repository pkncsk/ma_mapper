...
from ma_mapper._unused.fetch_data import extract_bed
...
#find peaks
import scipy
peaks, _ = scipy.signal.find_peaks(mean_clinvar, width = 6)
#highest_peak_index = peaks[np.argmax(mean_znf808[peaks])]
binding_indices = np.unique(np.where(clinvar[:, peaks] != 0)[0])
nonbinding_indices=list(set(np.arange(clinvar.shape[0])) - set(binding_indices))
#%%
# %%
...
import importlib
importlib.reload(plots)import importlib
importlib.reload(plots)
from ma_mapper import mapper
plots.plot_experimental(
    data = [phylop_sorted,clinvar_sorted,], 
