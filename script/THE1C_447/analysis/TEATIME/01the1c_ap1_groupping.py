...
plots.plot_experimental(
    ...
    heatmap_title=['AP-1 motif on THE1C MSA'],
    heatmap_title_fs = 10, 
...
plots.plot_experimental(
    ...
    hm_xhighlight=[[3,3],[-3,-3]],
    hm_xhighlight_col= ['grey','grey'],
    hm_xhighlight_alpha=[1,1,1],
    annotation_data=[age_anno_sorted,subgroup_anno_sorted],
    anno_cbar_label=[age_subgroups,['both','right only','left only','no motif/\nothers']],
    anno_cbar_title=['TEA-TIME','motif group'], 
    anno_cbar_even_pos=-0.19,
    colorbar=True,
    colorbar_steps=[0.1],
    heatmap_title=['phyloP of THE1C, grouped by AP-1 motif'],
    heatmap_title_fs = 10, 
    ...
    heatmap_xhighlight_alpha=[1,1,1],
    )
#%%
...
motif = 'AP-1'
pos_set = np.arange(304, 315)
fig = plt.figure(figsize=(10, 1 * len(pos_set)))
