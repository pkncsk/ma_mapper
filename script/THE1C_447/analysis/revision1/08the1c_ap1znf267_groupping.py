...
#%%
test_set[(test_set['ap1']=='A')&(test_set['znf267']=='A')].index
#%%
test_set[(test_set['ap1']=='A')&(test_set['znf267']=='B')].index
#%%
test_set[(test_set['ap1']=='B')&(test_set['znf267']=='A')].index
#%%
...
plots.plot_experimental(
    ...
    heatmap_title=['AP-1 motifs and ZNF267 signals on THE1C,\ngrouped by motifs and signals'],
    heatmap_title_fs = 10, 
    heatmap_xhighlight=[[426,426],[5573,5573],[5813,5813]],
...
plots.plot_experimental(
    ...
    anno_col = ['Blues',['purple','red','blue','white']], 
    annotation_data=[age_anno_sorted,subgroup_anno_sorted],
    anno_cbar_label=[age_subgroups,['both','ap1 only','znf267 only','no motif/\nsignals']],
    anno_cbar_title=['TEA-TIME','motif group'], 
    anno_cbar_even_pos=-0.19,
    colorbar=True,
    colorbar_steps=[0.1],
    heatmap_title=['phyloP of THE1C, grouped by AP-1 motifs and ZNF267 signals'],
    heatmap_title_fs = 10, 
    heatmap_xlabel = 'position (bp)',
    heatmap_xlabel_fs=10,
...
plots.plot_experimental(
    ...
    agg_colset=['blue','red'],
    agg_ylim=[[0,10],[-10,0]],
    agg_yhighlight=[[100,350],[212,232],[304,314]],
    agg_yhighlight_col= ['purple','purple','green'],
   ...
   agg_titles=['conserved\nsignal>no signal','accelerated\nsignal<no signal'], 
    ...
    agg_ylabel_xpos=0,
    agg_ylabel_fs=10,
    agg_plot_title=['phyloP of THE1C, with both AP-1 motifs and ZNF267 signals',None],
...
plots.plot_experimental(
   ...
   agg_titles=['conserved\nsignal>no signal','accelerated\nsignal<no signal'], 
    ...
    agg_ylabel_xpos=0,
    agg_ylabel_fs=10,
    agg_plot_title=['phyloP of THE1C, with only ZNF267 signals',None],
    agg_plot_title_fs= 12, 
    agg_xlabel = 'position (bp)',
    agg_xlabel_fs=10,
     agg_plottext=[None, f'TE w/ signals: {len(binding_indices)}\nTE w/o signals: {len(nonbinding_indices)}'],
