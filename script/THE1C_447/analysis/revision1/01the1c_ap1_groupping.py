...
plots.plot_experimental(
    ...
    heatmap_title=['AP-1 motifs on THE1C MSA'],
    heatmap_title_fs = 10, 
...
plots.plot_experimental(
    ...
    opacity = 0.8,
    hm_transparency_mode='gradient', 
    heatmap_title=['AP-1 motifs on THE1C, grouped by AP-1 motifs'],
    heatmap_title_fs = 10, 
    heatmap_xhighlight=[[536,536],[5573,5573],[5908,5908]],
...
plots.plot_experimental(
    ...
    anno_cbar_label=[age_subgroups,['both','only right','only left','no motif/\nothers']],
    anno_cbar_title=['TEA-TIME','motif group'], 
    anno_cbar_even_pos=-0.19,
    colorbar=True,
    colorbar_steps=[0.1],
    heatmap_title=['phyloP of THE1C, grouped by AP-1 motifs'],
    heatmap_title_fs = 10, 
    heatmap_xlabel = 'position (bp)',
    heatmap_xlabel_fs=10,
    anno_ylabel = 'sequences',
...
plots.plot_experimental(
    ...
    agg_ylabel_xpos=0,
    agg_ylabel_fs=10,
    agg_plot_title=['phyloP of THE1C, grouped by AP-1 motifs',None],
...
motif = jdb_obj.fetch_motif_by_id('MA0099.2')
#%%
import pandas as pd
import logomaker
...
ax.set_title('AP-1 motif from JASPAR', fontsize =12)
...
print(f'age_of_interest\tw/ motif\tw/o motif\tmost significant adjusted p-value')
...
plots.plot_experimental(
    ...
    agg_ylabel_xpos=0,
    agg_ylabel_fs=10,
    agg_plot_title=['phyloP of THE1C, with both major AP-1 motifs',None],
    agg_plot_title_fs= 12, 
    agg_xlabel = 'position (bp)',
    agg_xlabel_fs=10,
    agg_plottext=[None,f'TE w/ motifs: {len(binding_indices)}\nTE w/o motifs: {len(nonbinding_indices)}'],
...
plots.plot_experimental(
    ...
    agg_ylabel_xpos=0,
    agg_ylabel_fs=10,
    agg_plot_title=['phyloP of THE1C, with only the right major AP-1 motifs',None],
    agg_plot_title_fs= 12, 
    agg_xlabel = 'position (bp)',
    agg_xlabel_fs=10,
    agg_plottext=[None,f'TE w/ motifs: {len(binding_indices)}\nTE w/o motifs: {len(nonbinding_indices)}'],
...
plots.plot_experimental(
    ...
    agg_ylabel_xpos=0,
    agg_ylabel_fs=10,
    agg_plot_title=['phyloP of THE1C, with only the left major AP-1 motifs',None],
    agg_plot_title_fs= 12, 
    agg_xlabel = 'position (bp)',
    agg_xlabel_fs=10,
    agg_plottext=[None,f'TE w/ motifs: {len(binding_indices)}\nTE w/o motifs: {len(nonbinding_indices)}'],
...
for idx, age_of_interest in enumerate(age_of_interest_list):
    ...
    binding_indices = metadata_age_filtered[(metadata_age_filtered['right_group']=='B')&(metadata_age_filtered['left_group']=='A')].index
    nonbinding_indices = metadata_age_filtered[(metadata_age_filtered['right_group']=='B')&(metadata_age_filtered['left_group']=='B')].index
    if len(binding_indices) ==0 or len(nonbinding_indices) == 0:
        print(f'{age_of_interest}\t{len(binding_indices)}\t{len(nonbinding_indices)}\t{np.nan}')
    else:
        phylop_bind = phylop_447[binding_indices]
        phylop_nonbind = phylop_447[nonbinding_indices]
        alignemnt_bind=alignment_filtered[binding_indices]
