...
plt.setp(axs, ylim=(0,0.5), xmargin=0)
...
peaks, properties = find_peaks(fused_maf_averaged, prominence=-0.2)
plt.rcParams['figure.dpi'] = 600
plt.rcParams['savefig.dpi'] = 600
fig, ax = plt.subplots(figsize=(10,3))
ax.fill_between(range(len(fused_maf_averaged)), fused_maf_averaged, color='grey')
ax.margins(x=0, y=0)
ax.set_ylim(0,0.5)
ax.plot(peaks, fused_maf_averaged[peaks], "x")
ax.set_xlabel('position (bp)')
ax.set_ylabel('averaged alt allele ratio')
ax.set_title('MER11 MAF overlay alignment age calibration')
plt.show()
