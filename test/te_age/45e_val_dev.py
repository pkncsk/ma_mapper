...
e_cutoff=1e-3
#add flank fragments:
genostart_full=[min(genostart_list)-5000] + genostart_list + [max(genoend_list)]
genoend_full = [min(genostart_list)] + genoend_list + [max(genoend_list)+5000]
#%%    
...
spliced_maf_full =index_maf.get_spliced(genostart_full,genoend_full,strand)
#%%
collector = []
    ...
    if seqrec.id.split('.',maxsplit=1)[0] == target_species:
        ...
        for seqrec in spliced_maf_full:
            ...
            if matched != 0:
                ...
                if meta_name ==target_species:
                    print(count_result, blast_score,E)
                if E <= e_cutoff:
                    ...
                    if meta_name ==target_species:
                        print(count_result, blast_score,E,E_front,E_back)
...
#%%
def e_vel_core(seqrec, target_seq):
    meta_name = seqrec.id.split('.',maxsplit=1)[0]
    species_id = species_table[species_table['meta_name']==meta_name].index[0]
    chrom_name = seqrec.id.split('.',maxsplit=1)[1]
    #first pass - check TE E-value
    ...
    if matched != 0:
        ...
        if E <= e_cutoff:
            ...
            if (E_front <= e_cutoff or E_back <= e_cutoff):
                ...
                pgap_back=round((gap_count_back/5000*100), 2)
                E_score_back ='{:0.2e}'.format(E_back)
                return meta_name,chrom_name,age, iden, pgap, blast_score, E_score,[iden_front,iden_back],[pgap_front,pgap_back],[E_score_front,E_score_back]
#%%
def e_val_engine_full(chrom, strand, genostart_list, genoend_list, e_cutoff=1e-3):
    ...
    target_chrom = f'{target_species}.{chrom}'
    index_maf = MafIO.MafIndex(f'{mafpath}.mafindex', mafpath, target_chrom)
    spliced_maf_full =index_maf.get_spliced(genostart_list,genoend_list,strand)
    collector = []
    for seqrec in spliced_maf_full:
        if seqrec.id.split('.',maxsplit=1)[0] == target_species:
            target_seq = seqrec.seq
...
#%%
for internal_id in ['MER11A_SINGLE_126']:
    print(f'process {internal_id}')
    internal_id_tbl_subset = internal_id_tbl[internal_id_tbl.internal_id == internal_id]
    subset_index=internal_id_tbl_subset.rmsk_index.to_list()
    rmsk_subset=config.filtered_table[config.filtered_table.index.isin(subset_index)]
    chrom=rmsk_subset.genoName.unique()[0]
    ...
    genostart_full=[min(genostart_list)-5000] + genostart_list + [max(genoend_list)]
    genoend_full = [min(genostart_list)] + genoend_list + [max(genoend_list)+5000]
    E_table=e_val_engine_full(chrom, strand, genostart_full, genoend_full)
        # NOTE table join
...
for seqrec in spliced_maf_full:
    if seqrec.id.split('.',maxsplit=1)[0] == target_species:
        ...
        with ProcessPoolExecutor(max_workers=40) as executor:
            results = executor.map(e_vel_core, spliced_maf_full, itertools.repeat(target_seq))
collector = []
