...
# %%
age_div_tbl=pd.read_csv(f'/home/pc575/rds/rds-mi339-kzfps/users/pakkanan/phd_project_development/data/_mapper_output/hg38_repeatmasker/combined_age_div/all_subfamilies.txt', sep='\t')
...
for i in range(0,62):
    bins.append(i+0.9)
age_div_df['mya_binned'] = pd.cut(age_div_df['div_age_mya'], bins=bins, labels=list(range(0,62)))
...
for values, label in zip(age_count_by_class, repclass):
    main.bar(age_ref_table.representative_age[1:], values[1:], label=label, bottom=bottom, color=col_dict[label])
    bottom += values[1:]
