...
#%%
# %%
low_border_maf_mapped=fetch_data.fetch_maf(metadata_input= low_border_metadata, maf_input='/home/pc575/rds/rds-kzfps-XrHDlpCeVDg/users/pakkanan/241genomes/241-mammalian-2020v2b.maf', separated_maf = True,target_species = 'Homo_sapiens', custom_id= True, count_arg='common',age_arg='calibrate', age_table_file= age_table_filepath, age_list = age_list)
# %%
high_border_maf_mapped=fetch_data.fetch_maf(metadata_input= high_border_metadata, maf_input='/home/pc575/rds/rds-kzfps-XrHDlpCeVDg/users/pakkanan/241genomes/241-mammalian-2020v2b.maf', separated_maf = True,target_species = 'Homo_sapiens', count_arg='common',custom_id= True, age_arg='calibrate', age_table_file= age_table_filepath, age_list = age_list)
#%%
#%%
maf_front_list = []
maf_back_list = []
...
metadata_df_sorted_filtered[['subfam', 'subfam_order']] = metadata_df_sorted_filtered['id'].str.split('_', expand=True)
row_color_subfam_sorted=metadata_df_sorted_filtered.subfam.map(subfam_colorcode)
row_color_age_sorted=metadata_df_sorted_filtered.te_age.map(age_colorcode)
...
for label in metadata_df_sorted_filtered.te_age.unique():
...
for label in metadata_df_sorted_filtered.subfam.unique():
...
subfam_colorcode={'MER11A': 'red', 'MER11B':'blue', 'MER11C':'yellow'}
metadata_df_sorted_filtered[['subfam', 'subfam_order']] = metadata_df_sorted_filtered['id'].str.split('_', expand=True)
row_color_subfam=metadata_df_sorted_filtered.subfam.map(subfam_colorcode)
row_color_age=metadata_df_sorted_filtered.te_age.map(age_colorcode)
...
aligned_maf_clustered = []
for i in new_order:
    aligned_maf_clustered.append(aligned_maf_overlay[i])
aligned_maf_clustered = np.array(aligned_maf_clustered)
# %%
metadata_df_clustered=metadata_df_sorted_filtered.iloc[new_order]
# %%
maf_front_clustered = []
maf_back_clustered = []
for i in new_order:
...
for i in range(len(aligned_maf_overlay)):
    fused_maf_mapped_sorted.append(np.concatenate((maf_front_clustered[i], aligned_maf_clustered[i], maf_back_clustered[i])))
...
age_colorcode = {0:'#bf6f93',6.7:'#79b743',9.06:'#b262cb',15.76:'#cea240',20.19:'#6566d1',29.44:'#738139',43.2:'#ca489e',73.8:'#51ad7b',76:'#cf4766',82:'#45b0cf',90:'#d25337',96:'#7981c7',105:'#b97446'}
subfam_colorcode={'MER11A': 'red', 'MER11B':'blue', 'MER11C':'yellow'}
metadata_df_clustered[['subfam', 'subfam_order']] = metadata_df_clustered['id'].str.split('_', expand=True)
row_color_subfam_sorted=metadata_df_clustered.subfam.map(subfam_colorcode)
row_color_age_sorted=metadata_df_clustered.te_age.map(age_colorcode)
row_colors_sorted=pd.DataFrame({'subfam':row_color_subfam_sorted,'te_age':row_color_age_sorted}).reset_index(drop=True)
#%%
graphical_object_sorted=sns.clustermap(pd.DataFrame(fused_maf_mapped_sorted),row_colors=row_colors_sorted, row_cluster=False, col_cluster=False, cmap =  "viridis",xticklabels =500, yticklabels = 500, annot = False, vmin=0.98)
graphical_object_sorted.fig.subplots_adjust(left=0.05)
...
#graphical_object.ax_row_dendrogram.set_visible(False)
#graphical_object.ax_col_dendrogram.set_visible(False)
for label in metadata_df_clustered.te_age.unique():
    graphical_object_sorted.ax_row_dendrogram.bar(0, 0, color=age_colorcode[label], label=label, linewidth=0)
l1 = graphical_object_sorted.ax_row_dendrogram.legend(title='te_age', loc="upper right", bbox_to_anchor=(0.2, 0.8), bbox_transform=gcf().transFigure)
for label in metadata_df_clustered.subfam.unique():
    graphical_object_sorted.ax_col_dendrogram.bar(0, 0, color=subfam_colorcode[label], label=label, linewidth=0)
l2 = graphical_object_sorted.ax_col_dendrogram.legend(title='subfamily', loc="upper right", bbox_to_anchor=(0.205, 0.6), bbox_transform=gcf().transFigure)
...
plt.show()
#%%
metadata_nodex=metadata_df_clustered.reset_index(drop=True)
#%%
aligned_sorted = []
for i in new_order:
...
#%%
metadata_sorted_mer11a=metadata_nodex[metadata_nodex.subfam == 'MER11A']
metadata_sorted_mer11b=metadata_nodex[metadata_nodex.subfam == 'MER11B']
metadata_sorted_mer11c=metadata_nodex[metadata_nodex.subfam == 'MER11C']
...
plt.setp(axs, ylim=(0.97,1), xmargin=0)
